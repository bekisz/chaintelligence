...
- **`.env.secrets`** — gitignored secrets. Copy from `.env.secrets.example`. Holds `GRAPH_API_KEY`, `CMC_API_KEY`, `RPC_URL`, `DATA_WAREHOUSE_DB`, Airflow security keys, and `PORTAL_USERNAME`/`PORTAL_PASSWORD`.
...
DB-fetch performance and UI progress reporting on the routing endpoint, and Arbitrum DEX config correctness (factory addresses, init code hashes, token decimals). When changing DEX/network support, update `config/dex-config.yaml` and verify the per-network factory/init-hash there rather than hardcoding.
