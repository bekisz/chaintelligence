...
import logging
from common.utils.uniswap_utils import UniswapV4Fetcher, PostgresStorageV4, get_last_ingestion_ts
...
with DAG(
    'graph_arbitrum_uniswap_v4_swaps',
    max_active_runs=1,
    default_args=default_args,
    description='Fetch Uniswap V4 swaps on Arbitrum from The Graph',
    schedule='@hourly',
    start_date=pendulum.now().subtract(days=1),
    catchup=False,
    tags=['uniswap', 'swaps', 'defi', 'v4', 'arbitrum'],
) as dag:
    @task
    def fetch_and_store_swaps(**context):
        ...
        days = backfill_days.get('Arbitrum', 30)
        force_backfill = 'backfill_days' in conf
        network = 'Arbitrum'
        protocol = 'Uniswap V4'
        pg_hook = PostgresHook(postgres_conn_id='postgres_default')
        last_ts = get_last_ingestion_ts(pg_hook, network, protocol)
