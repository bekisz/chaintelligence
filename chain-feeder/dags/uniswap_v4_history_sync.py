    ...
    for r in rows:
        ...
        try:
            cur.execute("""
                ...
                VALUES ('Ethereum', 'Uniswap V4', %s, %s, %s, %s)
...
def build_daily_history():
    ...
    JOIN liquidity_pool p ON 
        p.network = 'Ethereum' AND p.protocol = 'Uniswap V4' AND
        p.fee_tier = CASE 
...
def sync_tvl_from_graph():
    ...
    # Priority 1: Swaps (Heuristic - most accurate for what's actually on chain/graph)
    ...
    # 2. Get all pools
    cur.execute("SELECT id, coin0_symbol, coin1_symbol, fee_tier FROM liquidity_pool WHERE protocol = 'Uniswap V4'")
    pools = cur.fetchall()
